...
declare module 'microgear' {
	...
	export interface Microgear extends Omit<EventEmitter, 'on'> {
		...
		on(event: 'warning', callback: (msg: string) => void): void;
		on(event: 'info', callback: (msg: string) => void): void;
		on(event: 'message', callback: (topic: string, msg: string) => void): void;
		on(event: 'present', callback: (event: Event) => void): void;
	}
	...
	function create(config: { key: string; secret: string; alias: string }): Microgear;
}
